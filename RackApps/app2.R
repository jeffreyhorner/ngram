...
app <- Rack::Builder$new(
	...
	'/graph' = function(env){
	    ...
	    x <- ngram_query2(content,wide=TRUE)
	    t <- paste(tempfile(),'.png',sep='')
	    png(filename=t,width=900,height=330)
            #p <- ggplot(data,aes(x=year,y=count))
	    #print(p + geom_line()) # + scale_y_log10() + ylab("Log (count)"))
	    image(x=x$year,z=as.matrix(x[,grep('X\\d+',names(x),value=TRUE)]),col=heat.colors(100))
	    #plot(x=data$year,y=data$count,type='l')
	    dev.off()
	    fi <- file.info(t)
	    body <- t
	    names(body) <- 'file'
	...
	'/?' = function(env){
	    req <- Rack::Request$new(env)
	    res <- Rack::Response$new()
	    res$redirect(req$to_url('/brew/app2.rhtml'))
	    res$finish()
	}
    )
)
